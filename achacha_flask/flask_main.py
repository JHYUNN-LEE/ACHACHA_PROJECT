...
import glob
import keras
# from yolo.yolo_prototype import Yolo
...
#     # crop 이미지 저장 코드
#     # im.save(f'/home/ubuntu/WEB_SERVICE_ACHACHA/FLASK/yolo/uploaded_file/real_{category}_pred_{name}_{time.strftime("%H_%M_%S")}.jpg')
#     return im
...
def extract(file):
    # global model
    file = file.convert('RGB').resize((224, 224))
...
def get_data():
    if request.method == "POST":
        ...
        result = search_img(category, img, threshold=threshold)
        ...
        print(f'총 소요시간 : {time.time() - yolo_start_time:.3f}초')
        print(result)
        print(len(result))
